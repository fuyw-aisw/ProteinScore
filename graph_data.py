...
class GoTermDataset(Dataset):
    ...
    def __init__(self, set_type, task, AF2model=False,esm1b=True,ood=True):
        ...
        self.processed_dir = '/data/ywfu/codes/Pythondata/ood-protein/data/prot30'
        if esm1b:
            if set_type == 'test' and not ood:
                self.graph_list = torch.load(os.path.join(self.processed_dir, f"{set_type}_graph_id.pt")) 
            elif set_type == 'AF2test' and not ood:
                self.graph_list = torch.load(os.path.join(self.processed_dir, f"{set_type}_graph_id.pt")) 
            else:
            ...
            if set_type == 'test' and not ood:
                self.graph_list = torch.load(os.path.join(self.processed_dir, f"{set_type}_graph_esm2_id.pt")) 
            elif set_type == 'AF2test' and not ood:
                self.graph_list = torch.load(os.path.join(self.processed_dir, f"{set_type}_graph_esm2_id.pt")) 
            else:
                self.graph_list = torch.load(os.path.join(self.processed_dir, f"{set_type}_graph_esm2.pt"))
                
        if set_type == 'AF2test' and ood:
            self.pdbch_list = torch.load(os.path.join(self.processed_dir, f"{set_type}_pdbch.pt"))["test_pdbch"]
        elif set_type == 'AF2test' and not ood:
            self.pdbch_list = torch.load(os.path.join(self.processed_dir, f"{set_type}_pdbch_id.pt"))["test_pdbch"]
        elif set_type == 'test' and not ood:
            self.pdbch_list = torch.load(os.path.join(self.processed_dir, f"{set_type}_pdbch_id.pt"))[f"{set_type}_pdbch"]
        else:
        ...
        if AF2model:
            ...
            if esm1b:
                ...
                graph_list_af = torch.load(os.path.join(self.processed_dir, f"AF2{set_type}_graph_esm2.pt"))                
